...
public class joystickCanvas extends View {
    ...
    protected void onSizeChanged(int w, int h, int oldW, int oldH) {
        ...
        ovalleft = (width / 2) - 500;
        ovalTop = (height / 2) - 700;
        ovalRight = (width / 2) + 500;
        ovalBottom = (height / 2) + 700;
        //Create  the oval
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                double ellipseBound = Math.pow(x - initPoint.x, 2) / Math.pow(500, 2) +
                        Math.pow(y - initPoint.y, 2) / Math.pow(700, 2);
                if (ellipseBound <= 1) {
                    isPlayerMoving = true;
                    newPoint.x = x;
                    newPoint.y = y;
